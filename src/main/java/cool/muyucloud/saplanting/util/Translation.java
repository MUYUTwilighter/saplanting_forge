...
public class Translation {
    ...
    public static boolean updateLanguage(String languageName) {
        String json;
        try {
            json = IOUtils.toString(
                Objects.requireNonNull(
                    Translation.class.getClassLoader()
                        .getResourceAsStream(String.format("assets/saplanting/lang/%s.json", languageName))),
                StandardCharsets.UTF_8);
        } catch (Exception e) {
            return false;
        }
        TRANSLATION.map = (new Gson()).fromJson(json, new TypeToken<HashMap<String, String>>() {}.getType());
        return true;
    }
}
